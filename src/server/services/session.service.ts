...
import { ChatMessage, Session, Json, ChatMessageAttachment, TokenUsage, SessionVersion } from "@/common/types";
...
export async function updateSessionCost(sessionId: string, costUsd: number, credits: number, userId: string) {
 ...
 */
export async function createSession(userId: string, name: string = "New Design"): Promise<Session> {
